...
router = APIRouter(
    prefix="/companies/{company_id}/scenarios",
    tags=["scenarios"],
...
def _check_scoring_dims(dims: list[str], company: models.Company) -> None:
...
def update_scenario(
    ...
    company = _get_company_or_404(company_id, db)
    scenario = _get_scenario_or_404(scenario_id, company_id, db)
